...
import { PlusCircle, Trash2, Send, Loader2 as Spinner, Search, X, CheckCircle2, Layers } from 'lucide-react';
...
interface FormItem extends Glaze {
  id: string;
  scope?: string;
...
export default function FormulatedWorkspace({ profile }: Props) {
  const uid = profile?.uid ?? auth.currentUser?.uid ?? null;
  const itemsPath = uid ? `formuladas/${uid}/items` : null;
  ...
  const [activeItem, setActiveItem] = useState<FormItem | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      q,
      (snap) => {
        setItems(snap.docs.map((d) => ({ ...(d.data() as FormItem), id: d.id })));
        setLoading(false);
      },
      (error) => {
        console.error('Error cargando área de Formuladas:', error);
        setLoading(false);
      },
    );
    return unsubscribe;
  ...
  const handleLoad = async (glaze: Glaze) => {
    ...
    try {
      ...
      setPicking(false);
      setFlash({ text: `"${glaze.name}" añadida a Formuladas`, key: Date.now() });
    } catch (error) {
      alert(error instanceof Error ? error.message : 'No se pudo añadir la ficha a Formuladas.');
    }
  };
  ...
  const handleRecalcSave = async (result: RecalcResult) => {
    ...
    try {
      ...
      setFlash({ text: `"${activeItem.name}" recalculada y guardada en Formuladas`, key: Date.now() });
    } catch (error) {
  ...
  const handlePublish = async (item: FormItem, status: GlazeStatus) => {
    if (!itemsPath && !item.id) return;
    const path = itemsPath as string;
  ...
  const handleDelete = async (item: FormItem) => {
    if (!itemsPath) return;
    if (!window.confirm(`¿Eliminar "${item.name}" de Formuladas?`)) return;
    try {
      await deleteDoc(doc(db, itemsPath, item.id));
      setFlash({ text: `"${item.name}" eliminada de Formuladas`, key: Date.now() });
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-lg font-semibold tracking-tight">Área de Formuladas</h3>
          <p className="text-xs text-[#636E72]">
            Las fichas que añadas se guardan aquí, fuera del repositorio, hasta que las publiques como borrador o
            publicado.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="flex min-h-[40vh] flex-col items-center justify-center rounded-[24px] border border-dashed border-[#E4E4E2] bg-white p-10 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#FBF7FA]">
            <Layers size={32} className="text-[#8a168a]" />
          </div>
          ...
          <p className="mt-2 max-w-md text-sm leading-relaxed text-[#636E72]">
            Añade fichas del repositorio a tu área de Formuladas. Solo al publicarla pasará al repositorio principal.
          </p>
          <button
            onClick={openPicker}
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    ...
                    title="Abrir recálculo de la fórmula"
                  >
                    <Send size={13} />
                    Recálculo
                  </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 p-4 backdrop-blur-sm">
            ...
            >
              <div className="flex items-center justify-between border-b border-[#F4F4F2] px-6 py-4">
                <div>
                  <h3 className="text-lg font-semibold tracking-tight">Añadir ficha a Formuladas</h3>
                  <p className="mt-0.5 text-xs text-[#85929E]">
                    Elige una ficha del repositorio para añadirla a tu área de Formuladas.
                  </p>
                </div>
                <button
